...
import com.picodiploma.newsapp.databinding.ItemNewsBinding
import com.picodiploma.newsapp.util.DateFormatter
...
class NewsAdapter(private val onBookmarkClick: (NewsEntity) -> Unit): ListAdapter<NewsEntity, NewsAdapter.MyViewHolder>(DIFF_CALLBACK) {
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        ivBookmark.setOnClickListener { onBookmarkClick(news) }
    }

    class MyViewHolder(internal val binding: ItemNewsBinding): RecyclerView.ViewHolder(binding.root){
        fun bind(news: NewsEntity){
            binding.tvItemTitle.text = news.title
            binding.tvItemPublishedDate.text = DateFormatter.formateDate(news.publishedAt)
